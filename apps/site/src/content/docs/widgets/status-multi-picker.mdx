...
| `summary` | `'chips' \| 'ellipsis' \| 'count' \| 'icons' \| 'names'` | `'ellipsis'` | What the control shows for the selection. `both` is the old spelling of `chips`. |
| `max` | `number` | `0` | Badges drawn before the rest becomes `+n`. `0` lets the row fit what it can, and `icons` draws twice as many. |
...
`chips` makes the control a token field: every badge, wrapped, each with its own remove control,
the caret beside them, and `+n` after the last badge `max` allows.
`icons` is the same field with the labels dropped and no per-badge remove, so removal happens in
the list; it fits twice as many.
`ellipsis` makes the control a one-line trigger: as many whole badges as the row fits, never a
cut one, then `+n` inline after the last, and the search box at the top of the popup.
`names` and `count` make it the same trigger, reading the labels joined by commas and
`3 selected`.
...
| any text | Narrows the rows, from the caret under `chips` and `icons` and from the popup's search box otherwise |
