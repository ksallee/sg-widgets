...
Single and multi: [StatusPicker](/widgets/status-picker/) takes one code.
...
label alone.
A row is the row anatomy of the design rules, after its checkbox: the status icon as the leading
glyph, the display label with the matched runs bold, and the code right-aligned. A status has one
display name and one icon, so the row's `labelField` and `thumbnail` are fixed, and it has no fields
of its own, so its secondary is a function rather than a field name. The control keeps the badges.
