...
them and have no page of their own; a picker pulls in its base, its row and its classes the way a
table pulls in its source binding and its footer. The same names exist in both registries.

| part | what it is | used by |
|---|---|---|
| `picker-control` | The control box and popup shell every picker is built on: the press rule, the caret, the keyboard model, inline and summary modes, the search row, the list, the state line and the load-more row. | entity, entity multi, status multi and entity type pickers, list multi select |
