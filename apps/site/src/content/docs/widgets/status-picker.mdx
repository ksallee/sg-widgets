...
It sits on the Select rather than the shared picker control: one code, no query and no chips,
and its press, its keyboard and its dismissal already match the pickers.
...
A row is the row anatomy of the design rules: the status icon as the leading glyph, the display label
as the main text, and the code right-aligned. A status has one display name and one icon, so the row's
`labelField` and `thumbnail` are fixed, and it has no fields of its own, so its secondary is a
function rather than a field name. The control keeps the badge.
