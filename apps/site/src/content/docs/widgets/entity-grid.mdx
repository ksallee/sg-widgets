...
<Demo name="entity-grid" title="Entity Grid: Versions at three tile sizes, a selectable grid, one with no picture, one with every third row disabled, and one drawing a card of the caller's own" />
