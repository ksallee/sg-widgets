...
This registry ships its own `command` for React, and its own `command`, `select` and `checkbox` for
Svelte. A host that already has a primitive of that name is asked before it is overwritten. Accept,
or pass `--overwrite`. A host that keeps its own `command` has no status part, and `search-control`
