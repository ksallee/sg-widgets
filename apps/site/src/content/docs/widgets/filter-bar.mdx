...
A pill names `maxValues` of what is ticked and reads the rest as `+n`, and its value is capped and
truncated with the whole list in its `title`, so a facet with everything ticked never stretches the
bar past the width it was given.

A facet over a status field draws a status badge for each value, in the checklist and in the pill,
in the site's own colour and with the site's own icon, so the bar reads as the lists beside it do. A
badge in the checklist marks what the search box matched, as every other list does.
