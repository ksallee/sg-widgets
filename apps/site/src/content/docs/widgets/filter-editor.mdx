...
| `emptyLabel` | `string` | `'Nothing chosen'` | Shown when a group holds no condition. |
...
| `list` | ListSelect on one value, ListMultiSelect on is any of and is none of |
...
in a ListSelect. Between draws two of the data type's editor. Is any of and is none of on a date, a
...
| `Enter` | Chooses the highlighted field, operator or value. In a multi-value list it checks the highlighted entry and the list stays open. |
| `Escape` | Closes the open menu or popover. |
| `Backspace` | In an entity picker's empty search box, arms the last chip; a second press removes it. |
