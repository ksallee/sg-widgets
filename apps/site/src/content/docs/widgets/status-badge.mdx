...
whose label is an entity's name, where a bordered pill reads as a second surface; a status listed as
an option of its own is the badge instead. The label stays as the accessible name and the tooltip,
`color` has nothing to paint, and a status with no icon to draw takes a neutral dot.

`query` draws the label's matched runs bold, so a badge standing as a row in a searched list reads as
the rows beside it do.
