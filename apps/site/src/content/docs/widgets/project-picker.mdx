...
Single and multi: [ProjectMultiPicker](/widgets/project-multi-picker/) takes several projects.
...
Every prop of [EntityPicker](/widgets/entity-picker/) except `entityTypes`, plus:

| prop | type | default | meaning |
|---|---|---|---|
| `includeArchived` | `boolean` | `false` | Offer projects whose `archived` checkbox is set. |

`placeholder` defaults to `Search for a project`.
...
The same as [EntityPicker](/widgets/entity-picker/).
