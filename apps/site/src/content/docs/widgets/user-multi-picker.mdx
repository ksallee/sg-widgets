...
Single and multi: [UserPicker](/widgets/user-picker/) takes one person.

## Install

```sh
pnpm dlx shadcn@latest add https://sg-widgets.dev/r/react/user-multi-picker.json
pnpm dlx shadcn-svelte@latest add https://sg-widgets.dev/r/svelte/user-multi-picker.json
```
...
Every prop of [EntityMultiPicker](/widgets/entity-multi-picker/) except `entityTypes`, plus:

| prop | type | default | meaning |
|---|---|---|---|
| `value` | `EntityRef[]` | `[]` | The chosen people, in the order they were ticked. Two-way in Svelte. |
...
The same as [EntityMultiPicker](/widgets/entity-multi-picker/): `Backspace` in an empty query arms
the last chip and a second press removes it, and the list stays open across ticks.
