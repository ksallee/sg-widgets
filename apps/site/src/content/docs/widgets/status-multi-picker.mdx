...
<Props name="status-multi-picker" />
...
label alone.
A row is the row anatomy of the design rules, after its checkbox, and a status offered as an option is
a badge: the icon and the display name in their pill, one step under the control, with the matched
runs bold and the code right-aligned after it. It is what the [filter bar](/widgets/filter-bar/) draws
...
own, so its secondary is a function rather than a field name. The control keeps its own badges for the
selection.
