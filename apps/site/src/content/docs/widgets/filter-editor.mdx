...
links, the coded lists, dates, and the flags and colours -- and each group spreads that family's
...
(017_filter_operators). Five data types take no filter operator at all — `url`, `calculated`,
`password`, `serializable` and `summary` — and answer `data type cannot be used in a filter` with no
legal set to name. A `pivot_column` advertises `is` and `is_not` and rejects both
(field_types/pivot_column). All six are left out of the field list.
...
buckets around today come from Dice UI's data table, its shadcn registry item, for the
`isRelativeToToday` date operator, mapped onto the calendar operators this API documents. None of
the three is installed.
