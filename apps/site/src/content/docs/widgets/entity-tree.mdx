...
<Demo name="entity-tree" title="Entity Tree: a project seeded open to one shot, with checkboxes, a filter and a thumbnail variant" />
...
The filter narrows the nodes already loaded, keeping every ancestor that leads to a match, and opens
those ancestors for as long as it is set.
...
| `onerror` / `onError` | `Error` | A read failed. |
...
The filter never asks the server, so a branch that was never opened is not searched.
