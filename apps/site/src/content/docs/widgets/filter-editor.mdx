...
| `size` | `'sm' \| 'md' \| 'lg'` | `'md'` | Condition-row heights 7, 8 and 9: a row is a control inside a control, so its ladder sits one step down. |
...
| event | payload | when |
|---|---|---|
| `onChange` | `FilterGroup` | The whole tree changed, on every edit. Svelte also binds it with `bind:value`. |
