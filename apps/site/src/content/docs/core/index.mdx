...
- **Client**: `RestClient` for the REST API, `ProxyClient` for an app's own endpoints with a matching server handler, `MockClient` for tests, and the context, the one object an app builds and hands to every widget: the cached client, the schema and status services, the site url and the site preferences.
