...
export const STOCK_ICON_DATA_URLS: Readonly<Record<string, string>> = {
  icon_airplane:
...
/**
 * Resolve an `image_map_key`. Status icons come from the bundle. A key outside the
 * status set is still a sprite cell, so it draws only when `siteUrl` is given.
 */
export function stockIconSource(imageMapKey: string | null | undefined, siteUrl?: string): StockIconSource {
  ...
  const data = STOCK_ICON_DATA_URLS[imageMapKey];
