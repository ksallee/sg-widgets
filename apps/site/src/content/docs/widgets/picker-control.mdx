...
<Demo name="picker-control" title="PickerControl: an inline single picker and a summary multi picker, over one static list" />

Both demos draw their two controls from a static list of departments at `md`: 136 lines of Svelte,
130 of React, chip renderer and row renderer included. The chip here is a plain text chip; a picker
that draws an entity chip or a status badge passes that instead.
...
`data-invalid`, and the ring the field takes from whatever inside it has focus. A readonly control
...
A chip carries `data-chip` so the measured row can weigh it.
...
| `Backspace` | In an empty query: the first arms the last chip, the second removes it. |
