...
- **Client**: `RestClient` for the REST API, `ProxyClient` for an app's own endpoints with a matching server handler, `MockClient` for tests, and `createSgContext`, the one object an app builds and hands to every widget.
