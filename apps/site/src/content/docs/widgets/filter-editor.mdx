...
| `projectId` | `number` | — | Scopes the status pickers to the codes one project allows. |
...
| `list` | ListSelect on one value, a checkbox list on is any of and is none of |
...
Inside a row the date, date-time and number editors take their inline form: the width the
data type needs, no calendar and no zone line. Between draws both ends on one line.

Arity comes first. An operator that carries its own value — is empty, is not empty, today, this week,
last month — draws no editor. In the last and in the next draw a count and a unit. Between draws two
of the data type's editor. Is any of and is none of on a date, a number or a text field draw one
...
| entity value | `entityEditor` | `entityEditor` snippet | The same arguments as the value editor. |
...
| `Right arrow` | Descends into the highlighted link in the field chooser. |
| `Left arrow` | Goes back one level in the field chooser. |
