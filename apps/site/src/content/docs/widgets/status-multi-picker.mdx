...
| `max` | `number` | `3` | Badges drawn before the rest becomes `+n`. `icons` draws twice as many. |
...
The control shows the selection as removable badges that wrap, as one clipped line of them under
`ellipsis`, as icon-only badges, as the names joined by commas, or as `3 selected`. Past `max` the
rest becomes `+n`, and `icons` fits twice as many. Without a project the options are the site
...
| any text | Opens the list and narrows the rows |
