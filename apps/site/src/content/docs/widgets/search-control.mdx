...
The highlight is the base's: it lands on the first row whenever the list changes, or on `leadKey`
where a wrapper draws a row of its own above the results. A load-more page moves it to the first row
of that page, so the list stays where the reader was instead of returning to the top. Matching is
the server's alone — the command box never filters what came back.
...
| `keyOf` | `(item) => string` | — | A row's key, which the list and the highlight are addressed by. |
| `leadKey` | `string` | `''` | The row the highlight lands on ahead of the first result. |
...
| `Escape` | Closes the dialog shell. |
