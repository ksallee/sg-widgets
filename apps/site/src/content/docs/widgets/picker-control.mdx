...
contents, not its box: the caller owns the list item, its selection state and anything it puts in
front, such as a checkbox.
...
One slot, `glyph` in both frameworks: it is drawn in the leading slot when the row carries no picture.
A row whose type is a person draws an avatar there instead.
