...
inset, at the size a list row draws a glyph (4 at sm and md, 5 at lg). It is what a leading slot
takes, where a bordered pill reads as a second surface. The label stays as the accessible name and
the tooltip, `color` has nothing to paint, and a status with no icon to draw takes a neutral dot.
