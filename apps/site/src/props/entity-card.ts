...
export default {
  props: [
    ...
    { name: 'subLabelField', type: '`FieldSpec | null`', default: '`null`', meaning: 'The left of the tile’s metadata line: a path, or a resolved column so it renders by type.' },
    { name: 'subLabel', type: '`(row) => string`', default: '—', meaning: 'The caller’s own sub-label. Wins over `subLabelField`.' },
    { name: 'secondaryField', type: '`FieldSpec | null`', default: '`null`', meaning: 'The right of the tile’s metadata line: a path, or a resolved column so it renders by type.' },
