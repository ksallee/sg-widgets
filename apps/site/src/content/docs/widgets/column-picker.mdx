...
A row is reordered by dragging its grip, which lifts the row and opens a gap where it will land, or
from the keyboard: Space on the grip picks the row up, the arrow keys move it, and Space drops it.
Every step is announced in a live region. Reduced motion keeps the reordering and drops the
movement.
