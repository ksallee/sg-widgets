...
It is the [list picker](/widgets/list-picker/) configured with a status row and a badge for its
value: the same control, popup, press rule and keyboard as every other picker.

Single and multi: [StatusMultiPicker](/widgets/status-multi-picker/) takes several codes.
...
| <kbd>Backspace</kbd> | In an empty search box, clears the status |
